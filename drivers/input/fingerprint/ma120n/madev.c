...
//Solution to restart the hw info can not read;Read the hardware manually before reset, not on the chip when the reset
//0表示成功；非0表示失败 add by gwc
int mas_read_hw(void) {
    int i, j, len = 4, ret = -1;
    const unsigned char cdat[] = { 0x04, 0xff, 0xff, 0xff };
    ...
    for(i=0;i<3;i++){
        ...
        if(ret == 0) {
            if(srxb[3] == 0x78) { //am120n
               
                break;
            } else if(srxb[3] == 0x79) { //am121n
...
int mas_probe(struct spi_device *spi) {
    ...
    mas_set_input();
    ret = mas_read_hw();
    printk("%s: mas_read_hw ret=%d\n", __func__, ret);
    if( ret != 0 ) {
        printk("%s: read hardware failed.\n", __func__);
    }
    MALOGF("end");
